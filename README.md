...
给 AI Agent 的 harness 做风洞实验：固定模型与任务，对 20 个 harness 模块做受控消融与反事实对比，经人的批准做可回滚演化。

**Agent = 模型 + harness。** 模型能力已经趋同，任务能不能可靠做完，取决于模型之外的 harness——循环、工具、记忆、上下文压缩、校验器、权限门。现有工具要么只回放单次轨迹，要么是黑盒自动改 harness 的研究代码。这里是一台**风洞**：用受控实验证清每个模块的边际作用，再把失败炼成一条经过验证、人批准、可回滚的配置改动。

## 两分钟看懂

同模型、同任务、两套 harness：**34% 对 77%**。差距不在模型。点进 12 步一键演示（`#/demo`），看一次失败是怎么变成一条配置改动的。

三屏一闭环：

1. **运行透视**（`#/insight`）——模型每一步看到了什么：分层上下文栈、五级压缩管线、校验器裁决、失败标签；可导入 Claude Code / dsh 真实轨迹回放，Harness 配置与 NLAH 双向互转。
2. **风洞**（`#/tunnel`）——拨 20 个模块的开关，多配置并排对比（成功率、相对锁定基线 Δ、token、时延、失败模式转移——双分支标注按步骤现算），下尾指标（MeanLift/WorstLift/RR₀/RelLift₉₅）与脆弱警告，模型适配画像与交叉矩阵，双轨迹逐步对齐到第一个分叉点。
3. **演化**（`#/evolution`）——失败聚类、改动卡（带预测区间）、风洞证伪、人批准、版本树提交、一键回滚。
4. **设置**（`#/settings`）——自带任意 LLM（12 家预设＋OpenAI 兼容自定义端点），密钥只驻留服务端内存。
5. **法律**（`#/legal`）——案件复盘、审计阶梯、合规牌照卡；**安全**（`#/security`）——攻击火线、控件消融、策略导出。
...
一键论文预设（风洞空状态载入）：**NLAH** 受控消融（arXiv:2603.25723v2, Table 5）、**AHE** 十代爬升 69.7%→77.0%（arXiv:2604.25850）、**Harness-R1** 反例 41.6%→35.4%（arXiv:2608.02276，附录 H.3）、**极简/丰富变量对**（fixture 示意读数）。
