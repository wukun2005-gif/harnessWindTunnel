    ...
    branches: [
      ...
      },
      {
        ...
        build(rec) {
          ...
          rec.req(ctx({ history: 9800, toolResult: { source: 'fs.read(interviews 4-6)', tokens: 3300, preview: 'The last 3 interviews crowd the window…' } }), 'History was not trimmed; continue reading the last 3 interviews')
          rec.tool('fs.write', 'report/section-1-2.md', 900, 'Sections 1–2 drafted')
          rec.req(ctx({ history: 14_100, toolResult: { source: 'fs.read(outline.md)', tokens: 300, preview: 'The outline is buried deep in earlier context; positional attention degrades' } }), 'Writing the risk section: section 2 figures contradict earlier text')
